address 09463 {
    module PersistenceDemo{
       ...
       // use 0x1::Testnet::is_testnet;
        ...
        spec initialize{
          let adr = Signer::address_of(sender);
          ensures exists<State>(adr);
          ensures len(global<State>(adr).hist) == 0;
        }
        ...
        spec add_stuff{
          let adr = Signer::address_of(sender);
          ensures exists<State>(adr);
          ensures len(global<State>(adr).hist) == old(len(global<State>(adr).hist)) + 3;
        }
